...
var connection = mysql.createConnection({
  ...
  figlet('Department Tracker', (err, result) => {
  ...
  viewSomething = () => {
    inquirer.prompt([
      ...
      else if (answer.viewChoice === "EMPLOYEES BY MANAGER") {
        console.log('\r\n');
  ...
  viewRoles = () => {
    x = `
    SELECT r.id AS ID, r.title AS Title, CONCAT('$', Format(r.salary, 2)) AS Salary, d.name AS "Department Name"
  ...
  viewEmployees = () => {
    x = `
      SELECT e.id AS ID, e.first_name AS "First Name", e.last_name AS "Last Name", d.name AS Department, 
      r.title AS Title, CONCAT('$', Format(r.salary, 2)) AS Salary, 
      CONCAT_WS(" ", m.first_name, m.last_name) AS Manager 
      FROM employee e LEFT JOIN employee m ON m.id = e.manager_id INNER JOIN role r ON e.role_id = r.id 
  ...
  viewEmployeesByManager = () => {
    x = `
    SELECT a.id AS ID, CONCAT_WS(" ",a.first_name, a.last_name) AS Manager, 
    CONCAT_WS(" ", b.first_name, b.last_name) AS Employee
    FROM employee a join employee b ON a.id = b.manager_id
    ORDER BY a.id
  `
  connection.query(x, (err, res) => {
    if (err) throw err;
  ...
  viewDeptSalary = () => {
    x = `
    SELECT d.name AS Department, CONCAT('$', Format(SUM(r.salary), 2)) AS "Department Salary" 
